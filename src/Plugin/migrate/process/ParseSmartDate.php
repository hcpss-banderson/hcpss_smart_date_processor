...
/**
 * We only need this processor because the one included with smart_dates has
 * some bugs:
 *
 * https://www.drupal.org/project/smart_date/issues/3475105
 * https://www.drupal.org/project/smart_date/issues/3475123
 *
 * Once those bugs are fixed, we can remove this plugin and use the smart_date
 * parse_date processor.
 *
 *
 * @MigrateProcessPlugin(
 *   id = "hcpss_parse_smart_dates"
 * )
 */
class ParseSmartDate extends ParseDates {
  protected const MIDNIGHT = '235959';
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    foreach ($value as $k => $d) {
        ...
        if (strpos($d, 'date-display')) {
          ...
          if (isset($m[4])) {
            if (strpos($m[4], 'times')) {
              ...
              $rrule .= ';UNTIL=' . $this->date('Y-m-d\THis', strtotime(substr($m[4], 6)));
            }
          }
          ...
          if (!preg_match('#"date-display-start"[^>]*>([^<]*).*"date-display-end"[^>]*>([^<]*)#', $d, $m)) {
            ...
            $m[2] = ($this->date('H:i:s', $m[1]) == '00:00:00') ? ($m[1] + self::ONE_DAY - 60) : $m[1];
          }
          $d = [
            'value' => $m[1],
            'value2' => $m[2],
      ...
      if (!isset($d['rrule']) && $d['value2'] - $d['value'] > self::ONE_DAY) {
        ...
        $end = $d['value2'];
        $d['rrule'] = 'RRULE:FREQ=DAILY;INTERVAL=1;UNTIL=' . $this->date('Y-m-d\T', $end).self::MIDNIGHT;
        $d['value2'] = strtotime($this->date('Y-m-d', $d['value']) . $this->date(' H:i:s', $d['value2']));
      ...
      if (isset($d['rrule']) && $d['rrule'] > '') {
        ...
        if (!isset($rdata['rule']) || $d['rrule'] != $rdata['rule']) {
          ...
          elseif ($m[3] == 'UNTIL') {
            ...
            if ($enddate > 2147483647) {
              $limit = 'UNTIL=' . $this->date('Y-m-d\THis', 2147483647);
              $newrule = str_replace($m[2], $limit, $d['rrule']);
            }
          }
          elseif (!isset($m[2])) {
   ...
   * @throws \Exception
   */
  private function date($format = "r", $timestamp = FALSE): string {
    $gmtTimezone = new \DateTimeZone('UTC');
    $myDateTime = new \DateTime(($timestamp ? date("r", (int) $timestamp) : date("r")), $gmtTimezone);
    return date($format, ($timestamp ? (int) $timestamp : $myDateTime->format('U')));
  }
}
